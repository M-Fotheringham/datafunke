...
def auc_grabber(x, y):
    # Calculate area from ordered x (fpr) and y (tpr) points
    auc = abs(round(np.trapz(y=y, x=x), 3))
...
def threshold_grabber(thresh_vals, outcome_vals, true_labels=None):
            ...
            set(
                [
                    ...
                    for idx, x in enumerate(
                        [*df["thresh_vals"], max(df["thresh_vals"]) + 1]
                    )
                ]
            )
        )
    ...
    auc = auc_grabber(x=fpr_list, y=tpr_list)
    ...
    quantiles = [
        percentileofscore(df["thresh_vals"], thresh, kind="rank")
...
def roc_plotter(
    ...
    if youden:
        ...
        youden_fpr = thresh_df[thresh_df["Youden_idx"] == youden_idx][
            "fpr"
        ].reset_index(drop=True)
        youden_tpr = thresh_df[thresh_df["Youden_idx"] == youden_idx][
            "tpr"
        ].reset_index(drop=True)
